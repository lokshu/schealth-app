<template>
	<view class="index-list-wrap" :style="{height: llHieght + 'px'}">
		<scroll-view scroll-y="true" style="height: calc(100%);" :scroll-into-view="scrollIntoView" :scroll-with-animation="scrollWithAnimation"
		 @scroll="onScroll" scroll-anchoring>
			<view v-for="(item, index) in list" :key="genIndexKey(item[indexKey],index)" :id="item[indexKey]" class="part"
			 :data-id="item[indexKey]" @click.stop="">
				<view class="part-title" :style="{color: currentFlag === item[indexKey] ? activeColor : ''}" @click.stop="">
					{{item[indexKey]}}
				</view>

				<view :class="['item', {disabled: citem.disabled}]" v-for="(citem, cindex) in item[dataKey]" :key="genIndexKey(citem, cindex)"
				 :data-cindex="cindex" @click.stop="itemClick(citem, index, cindex)">
		...
		<view v-if="showIndex" class="index-part" >
			...
			<view class="shadow-layer" @click.stop="shadowLayerClick" @touchmove="shadowLayerMove">
			</view>
		</view>
		<!-- 显示点击索引 -->
		<view class="show-select-flag" v-show="showCFlag">
			{{currentFlag}}
		</view>
	</view>
</template>
...
<script>
	...
	export default {
		...
		mounted() {
			this.llHieght = this.lHeight
			if (!this.llHieght) {
				uni.getSystemInfo({
					success: (obj) => {
						this.llHieght = obj.windowHeight
					}
				})
			}
		},
