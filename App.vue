<!-- <template>
	<view class="bg-view bg-6D14B3" style="max-width: 800px; min-height: 100vh;" v-if="globalData.data.loading">
		<view class="" id="loader">
		  <view id="shadow"></view>
		  <view id="box"></view>
		</view>
	</view>
...
<script>
	...
	export default {
		globalData: {
			...
			// #ifdef APP-PLUS
			// img_host: 'http://mall.tgyunchuang.com',
			img_host: 'https://egogo.hk',
			// #endif
			// #ifdef H5
			// img_host: location.hostname == 'localhost' ? 'http://mall.tgyunchuang.com' : location.origin,
			//img_host: location.hostname == 'localhost' ? 'https://egogo.hk' : location.origin,
			img_host: location.hostname == 'localhost' ? location.origin : location.origin,
			// #endif
			/**
			 * 启动参数处理
			 */
							...
							uni.request({
								...
								success: res => {
									...
									if (res.data.code == 0) {
										...
										if ((data.is_user_exist || 0) == 1 || client_type ==
											'weixin') {
											uni.setStorage({
				...
				if (this.is_weixin_env()) {
					...
					if (web_openid == null) {
						...
						if (params != null && (params.is_weixin_auth_web_openid || 0) == 1) {
							...
							uni.setStorageSync(this.data.cache_page_pay_key, (typeof order_ids == 'array') ? order_ids
								.join(',') : order_ids);
							var page_url = this.get_page_url();
			...
			share_content_handle(data) {
				...
				// #ifdef H5
				result['url'] = result.url.split('#')[0] + '#' + ((result.path.substr(0, 1) == '/') ? '' : '/') + result
					.path + result.query;
				// #endif
				return result;
			},
			...
			user_auto_login_handle() {
				...
				if (user == null) {
					...
					var url = null;
					// 是否微信环境
					if ((params.thirdpartylogin || null) == null && config != null && data != null && this
						.is_weixin_env()) {
		...
		onLaunch(params) {
			...
			// let item = uni.getStorageSync('lang');
			// let lang = uni.getStorageSync('lang') == null ? 'cht' : uni.getStorageSync('lang');
			// if (uni.getStorageSync('lang_list')) {
			...
			// 			this.globalData.data.loading = false;
			// 			uni.setStorageSync('lang', lang)
			// 			uni.setStorageSync("lang_list", JSON.stringify(res.data.data))
			// 			this.$i18n.locale = uni.getStorageSync('lang')
			// 			// this.$i18n.mergeLocaleMessage(uni.getStorageSync('lang'), res.data.data)
			// 			this.$i18n.mergeLocaleMessage(uni.getStorageSync('lang'), JSON.parse(uni
			// 				.getStorageSync('lang_list')))
			// 		} else {
			// 			app.globalData.showToast("服務器請求出錯，請刷新後重試");
			// 		}
			// 	},
			// 	fail: () => {
		...
		onShow(params) {
			...
			uni.setTabBarItem({
				index: 1,
				text: this.$t('TABBAR.HEALTH')
			});
			uni.setTabBarItem({
				index: 2,
				text: this.$t('TABBAR.APPOINTMENT')
			});
			uni.setTabBarItem({
				index: 3,
			...
			// console.log(localStorage.cart_length)
			if (!uni.getStorageSync('user_token')) {
				// localStorage.removeItem('cart_length')
				uni.removeStorageSync('cart_length');
			} else {
				let cart_total = uni.getStorageSync('cart_length')
				if (uni.getStorageSync('cart_length') <= 0 || !uni.getStorageSync('cart_length')) {
					this.globalData.set_tab_bar_badge(2, 0);
				} else {
					this.globalData.set_tab_bar_badge(2, 1, cart_total);
				}
			}
			// let item = uni.getStorageSync('lang');
			let lang = uni.getStorageSync('lang') == null ? 'cht' : uni.getStorageSync('lang');
			...
			// 				.getStorageSync('lang_list')))
			// 		} else {
			// 			app.globalData.showToast("服務器請求出錯");
			// 		}
			// 	},
			// 	fail: () => {
...
<style>
	...
	/* @font-face {
		font-family: my-font;
		src: url('@/static/fonts/SourceHanSansSC-Regular-2.otf');
	} */

	@font-face {
		font-family: my-fonts;
		src: url('@/static/fonts/DINCondensedC-Regular-2.ttf');
	}

	uni-tabbar .uni-tabbar__bd ,
	::v-deep uni-tabbar .uni-tabbar__bd {
		height: 120rpx !important;
	}

	html,
	body {
		font-family: 'Open Sans', 'pingfang SC', 'helvetica neue', arial, 'hiragino sans gb', 'microsoft yahei ui', 'microsoft yahei', simsun, sans-serif;
	}
	...
	::v-deep .uni-tabbar .uni-tabbar__item .uni-tabbar__icon img{object-fit: contain;}
	::v-deep .uni-tabbar .uni-tabbar__item:nth-child(4) .uni-tabbar__icon {
	...
	::v-deep .uni-tabbar .uni-tabbar__badge {
		height: 30rpx;
		line-height: 30rpx;
		min-width: 30rpx;
		font-size: 22rpx;
	...
	.top-title-banner1 .top-back-btn,
	.top-title-banner .top-back-btn{
		background-color: #fff;
		border-radius: 100%;
		padding: 8rpx;
	}
	.top-title-banner .left-part{
		color: #6D14B3;
		font-size: 30rpx;
	}
	.top-title-banner .left-part .name{
		font-weight: 600;
		font-size: 40rpx;
		margin-bottom: 10rpx;
	}
	.top-title-banner .right-part navigator{
		display: inline-block;
		position: relative;
		margin: 10rpx 0 10rpx 25rpx;
	}
	.top-title-banner .right-part navigator uni-text{
		position: absolute;
